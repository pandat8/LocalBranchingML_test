...
class LocalBranching:
    ...
    def __init__(self, MIP_model, MIP_sol_bar, k=20,  node_time_limit=30, total_time_limit=3600):
    ...
    def left_branch(self, time_limit, is_symmetric=True):
        self.create_subMIP()
        ...
        self.subMIP_model.setParam('limits/time', time_limit)
    ...
    def step_localbranch(self, k, node_time_limit):
        ...
        reward_t = 1 - t_leftbranch / node_time_limit
        reward_obj = np.abs(MIP_obj_best_pre - self.MIP_obj_best)/ np.maximum(np.abs(MIP_obj_best_pre), np.abs(self.MIP_obj_best))

        state[5:7] = [reward_t, reward_obj]
        return state

    def policy_vanilla(self, state):

        if state[0:4] == [1, 0, 0, 0]:
            k_step = [0, 0, 0, 1]
            t_step = []

    def mdp_localbranch(self, is_symmetric=True, reset_k_at_2nditeration=False):
        ...
        while self.total_time_available > 0 and self.k < self.n_binvars:  # and self.div < self.div_max
            ...
            state = self.step_localbranch(k=self.k, node_time_limit=node_time_limit)
    ...
    def search_localbranch(self, is_symmetric=True, reset_k_at_2nditeration=False):
        ...
        while self.total_time_available > 0 and self.k < self.n_binvars: # and self.div < self.div_max
            ...
            node_time_limit = np.minimum(node_time_limit ,self.total_time_available)
            self.left_branch(node_time_limit, is_symmeric=self.is_symmetric) # execute 1 iteration of lb
